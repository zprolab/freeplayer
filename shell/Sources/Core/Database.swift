...
enum Database {
    ...
    static func resetDatabase() -> Bool {
        ...
        // A reset must also remove the managed single-file library. Keep the
        // legacy filesystem library untouched so disabling the plugin remains
        // reversible, but never leave orphaned fpmlib audio behind.
        if let library = getSetting("library_dir", nil),
           getSetting("managed_library_enabled", "1") == "1" {
            let container = FPMLib.containerPath(library: library)
            try? FileManager.default.removeItem(atPath: container)
            DebugLog.file("RESET removed fpmlib container=\(container)")
        }
        return true
    }
