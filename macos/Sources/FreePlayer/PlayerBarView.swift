    ...
    private func button(action: @escaping () -> Void, icon: String, size: CGFloat, title: String) -> some View {
        Button(action: action) {
            Image(systemName: icon)
                ...
                .background(hoveringButton == icon ? Color(red: 0.910, green: 0.910, blue: 0.910) : .clear)
    ...
    private func playModeButton(_ mode: PlayMode, icon: String, title: String) -> some View {
        Button {
            ...
            Image(systemName: icon)
                ...
                .background(hoverMode == mode && model.playMode != mode ? Color(red: 0.910, green: 0.910, blue: 0.910) : .clear)
                .clipShape(RoundedRectangle(cornerRadius: 4))
        }
        .buttonStyle(.plain)
